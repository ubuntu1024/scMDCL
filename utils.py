...
import opt
from ops_loss import SupConLoss
...
def reconstruction_loss(X, A_norm, Z_hat, A_hat):
    ...
    loss_igae = loss_w + opt.args.alpha1 * loss_a
    loss_rec =  loss_igae
    return loss_rec
...
def distribution_loss(Q, P):
    loss = F.kl_div((Q[0].log() + Q[1].log()) / 2, P, reduction='batchmean')
...
def off_diagonal(x):
    n, m = x.shape
    assert n == m
    a=x.flatten()[:-1].view(n - 1, n + 1)[:, 1:].flatten()
...
def FeCL_loss(cons,Q=None):
    criterion= SupConLoss(temperature=0.01, cuda_device='0')
    if Q is None:
        L_N = criterion(cons[0],cons[1])
    else:
        y_pred = torch.argmax(Q, dim=1).data.cpu().numpy()
        if len(cons)==1:
            L_N = criterion(feature1=cons[0], labels=torch.tensor(y_pred))
        else:
            L_N = criterion(cons[0], cons[1], labels=torch.tensor(y_pred))
